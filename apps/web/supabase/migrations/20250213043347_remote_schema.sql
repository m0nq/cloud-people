...
ALTER TABLE ONLY "public"."Executions"
    ADD CONSTRAINT "Executions_agent_id_fkey" FOREIGN KEY ("agent_id") REFERENCES "public"."Agents"("id");
...
CREATE POLICY "Executions Workflow Owner Access" ON "public"."Executions" TO "authenticated" USING ((EXISTS ( SELECT 1
  ...
  WHERE (("n"."id" = "Executions"."agent_id") AND ("w"."user_id" = "auth"."uid"()))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."Nodes" "n"
     JOIN "public"."Workflows" "w" ON (("n"."workflow_id" = "w"."id")))
  WHERE (("n"."id" = "Executions"."agent_id") AND ("w"."user_id" = "auth"."uid"())))));
