...
-- Create function to update tool usage
...
-- Create trigger for tool usage tracking
DROP TRIGGER IF EXISTS track_tool_usage ON "public"."AgentTools";
CREATE TRIGGER track_tool_usage
    BEFORE INSERT OR UPDATE ON "public"."AgentTools"
    FOR EACH ROW
    EXECUTE FUNCTION update_tool_usage();

-- Add primary key to Tools
...
CREATE INDEX IF NOT EXISTS "idx_tools_name_version" ON "public"."Tools" ("name", "version");
CREATE INDEX IF NOT EXISTS "idx_agent_tools_usage" ON "public"."AgentTools" ("last_used_at", "usage_count");
