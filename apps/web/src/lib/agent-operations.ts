...
export const pauseAgentExecution = async (
    ...
    try {
        ...
        const browserUseEndpoint =
            process.env.NEXT_PUBLIC_BROWSER_USE_ENDPOINT || 'http://localhost:8000';
        ...
        console.log(
            `[DEBUG] Checking if task ${taskId} exists at ${browserUseEndpoint}/execute/${taskId}`
        );
        ...
        try {
            ...
            try {
                ...
                if (listResponse.ok) {
                    ...
                    if (ourTask) {
                        console.log(
                            `[DEBUG] Found our task in active tasks list:`,
                            ourTask
                        );
                    } else {
                        console.warn(
                            `[DEBUG] Our task ${taskId} not found in active tasks list`
                        );
                    }
                } else {
                    console.warn(
                        `[DEBUG] Failed to list active tasks: ${listResponse.status}`
                    );
                }
            } catch (listError) {
            ...
            const checkResponse = await fetch(
                `${browserUseEndpoint}/execute/${taskId}`
            );
            ...
            if (!checkResponse.ok) {
                if (checkResponse.status === 404) {
                    console.warn(
                        `[DEBUG] Task ${taskId} not found (404). The task may have completed or been closed.`
                    );
                    ...
                    console.error(
                        `[DEBUG] Task not found or cannot be accessed: HTTP ${checkResponse.status}`
                    );
                    throw new Error(
                        `Task not found or cannot be accessed: HTTP ${checkResponse.status}`
                    );
                }
            }
            ...
            if (taskStatus.status !== 'running') {
                console.log(
                    `[DEBUG] Task ${taskId} is not in running state (current state: ${taskStatus.status}), marking as paused`
                );
            ...
            console.log(
                `[DEBUG] Sending pause request to ${browserUseEndpoint}/execute/${taskId}/pause`
            );
            const response = await fetch(
                `${browserUseEndpoint}/execute/${taskId}/pause`,
                {
                    method: 'POST',
                    headers: {
            ...
            if (!response.ok) {
                ...
                if (
                    errorMessage.includes('session was lost') ||
                    errorMessage.includes('cannot be paused')
                ) {
            ...
            if (error instanceof Error && error.message.includes('404')) {
                console.warn(
                    `[DEBUG] Task ${taskId} not found (404) during pause operation. The task may have completed or been closed.`
                );
...
export const pauseAgentNode = async (
    ...
    try {
        ...
        const { getAgentData, getAgentState, updateAgentState } =
        ...
        if (!agentData) {
            ...
            // Create minimal agent data with just the nodeId
            ...
            // Store this minimal agent data to prevent future errors
            const { setAgentData } = useAgentStore.getState();
            setAgentData(agentId, minimalAgentData);
            ...
            updateAgentState(agentId, {
                ...
                metadata: {
                    pausedAt: new Date().toISOString(),
                    pauseReason: 'Agent data not found during pause operation'
                }
            });
        ...
        console.log(
            `[DEBUG] Current agent state: ${getAgentState(agentId)?.state || 'unknown'}`
        );
        ...
        try {
            ...
            const success = await pauseAgentExecution(
                agentId,
                updatedAgentData,
                onStatusChange
            );
            ...
            if (success) {
                ...
                return {
                    ...
                    success: true, // Consider this "successful" in terms of pausing the workflow
                    error:
                        'Failed to pause agent execution, but node transitioned to Paused state'
                };
            }
        } catch (error) {
            console.error(
                `[DEBUG] Error in pauseAgentExecution for node ${nodeId}:`,
                error
            );
